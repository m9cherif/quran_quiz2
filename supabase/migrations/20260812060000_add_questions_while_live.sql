-- Hosts may add questions (and edit never-started ones) while a game is
-- live: the control room gains an "Add question" dialog that writes through
-- save_question. A question whose window has already started is locked —
-- re-creating its choices would invalidate submitted answers (FK set null).
