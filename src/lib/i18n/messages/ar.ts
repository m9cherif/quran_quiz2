import type { Messages } from "../types";

/** ar dictionary — loaded on demand by I18nProvider (en is the static default). */
const ar: Messages = {
    common: {
      save: "حفظ",
      saving: "جارٍ الحفظ…",
      cancel: "إلغاء",
      close: "إغلاق",
      delete: "حذف",
      open: "فتح",
      archive: "أرشفة",
      create: "إنشاء",
      join: "انضمام",
      leave: "مغادرة",
      copy: "نسخ",
      retry: "إعادة المحاولة",
      back: "رجوع",
      tryAgain: "حاول مجددًا",
      confirm: "تأكيد",
      loading: "جارٍ التحميل…",
      code: "الرمز",
      players: "لاعبًا",
      points: "نقطة",
      finished: "منتهية",
      cancelled: "ملغاة",
      running: "جارية",
      waiting: "في انتظار",
      paused: "متوقفة مؤقتًا",
      noGamesYet: "لا توجد مسابقات بعد",
      noResults: "لا توجد نتائج",
      loadFailedTitle: "تعذّر تحميل هذه الصفحة",
      loadFailedDesc: "تحقق من اتصالك وحاول مجددًا.",
      draft: "مسودة",
      scheduled: "مجدول",
      reset: "إعادة ضبط",
      questionWord: "سؤالًا",
      answerWord: "إجابات",
      avg: "متوسط",
      errorTitle: "حدث خطأ ما",
      errorDesc: "حدث خطأ غير متوقع أثناء عرض هذا القسم.",
    },
    nav: {
      practice: "استمع وتابع",
      home: "الرئيسية",
      about: "عن المنصة",
      myQuizzes: "اختباراتي",
      liveGames: "المسابقات المباشرة",
      classes: "الصفوف",
      analytics: "التحليلات",
      timeline: "محرّر التوقيتات",
      newQuiz: "اختبار جديد",
      dashboard: "لوحة البيانات",
      series: "السلاسل",
      history: "السجل",
      joinGame: "انضم إلى مسابقة",
      logIn: "تسجيل الدخول",
      signOut: "تسجيل الخروج",
      host: "مضيف",
      adminOverview: "الإدارة",
      adminUsers: "المستخدمون",
      adminCompetitions: "المسابقات",
      toggle: "تبديل القائمة",
      main: "القائمة الرئيسية",
      mobile: "قائمة الجوال",
    },
    landing: {
      heroTitle: "مسابقات مباشرة مصممة للفصل الدراسي",
      heroSubtitle:
        "أطلق جولات أسئلة مباشرة مع طلابك، أو انضم إلى مسابقة برمز. لا تنزيلات ولا إعداد — فقط أسئلة وإجابات ولوحة صدارة حية.",
      hostQuiz: "استضافة مسابقة",
      rolesHeading: "اختر تجربتك",
      hostTitle: "استضافة مسابقة",
      hostDescription:
        "أنشئ مسابقة، ابدأ مسابقة مباشرة، وشاهد لوحة الصدارة تتحدث آنيًا.",
      startHosting: "ابدأ الاستضافة",
      joinTitle: "انضم إلى مسابقة",
      joinDescription: "أدخل الرمز الذي شاركه معلمك، اختر اسمًا، وشارك مباشرة على أي جهاز.",
      enterCode: "أدخل رمزًا",
      stepsHeading: "كيف تعمل",
      step1Title: "أنشئ أو افتح مسابقة",
      step1Description: "ابنِ أسئلتك، أو اختر مسابقة جاهزة من مكتبتك.",
      step2Title: "شارك رمز المسابقة",
      step2Description: "ينضم الطلاب إلى قاعة الانتظار بالرمز عبر هواتفهم.",
      step3Title: "شارك في الوقت الفعلي",
      step3Description: "تظهر الأسئلة للجميع دفعة واحدة، وتُحدَّث النقاط مباشرة.",
    },
    about: {
      title: "عن المنصة",
      p1: "منصة مسابقات القرآن أداة تعليمية تفاعلية فورية. ينشئ المعلمون المسابقات ويستضيفون مسابقات مباشرة برمز انضمام مشترك؛ ويجيب الطلاب على أجهزتهم بينما تتحدث النقاط ولوحة الصدارة فورًا.",
      p2: "تُجهَّز المنصة لمحتوى عربي أولًا، بما في ذلك أسئلة حفظ القرآن مع مراجع السورة والآية، ودعم كامل للاتجاه من اليمين إلى اليسار.",
      p3: "يعمل كل شيء عبر Supabase — المصادقة وقاعدة البيانات والتحديثات الفورية — فتبقى الأسئلة والإجابات وحالة المسابقة متزامنة دون بنية إضافية.",
    },
    auth: {
      welcomeBack: "مرحبًا بعودتك",
      logInSub: "سجّل الدخول لإدارة اختباراتك ومسابقاتك.",
      email: "البريد الإلكتروني",
      password: "كلمة المرور",
      createAccount: "إنشاء حساب",
      registerSub: "اختر دورك للبدء.",
      name: "الاسم الكامل",
      iAmHost: "أنا مضيف (معلم)",
      iAmStudent: "أنا طالب",
      iAmAdmin: "أنا مسؤول",
      alreadyHaveAccount: "لديك حساب بالفعل؟",
      noAccount: "ليس لديك حساب؟",
      forgotLink: "نسيت كلمة المرور؟",
      needAccount: "تحتاج حسابًا؟",
      signIn: "تسجيل الدخول",
      signUp: "إنشاء حساب",
      invalidCredentials: "البريد الإلكتروني أو كلمة المرور غير صحيحة. حاول مرة أخرى.",
      notConfigured: "Supabase غير مهيّأ على هذا النشر.",
      couldNotSignIn: "تعذّر تسجيل الدخول.",
      serverUnreachable: "تعذّر الوصول إلى الخادم. تحقق من اتصالك وحاول مجددًا.",
      accountCreatedTitle: "تم إنشاء الحساب",
      accountCreatedDesc: "جارٍ تسجيل دخولك…",
      nameTooShort: "يجب ألا يقل الاسم عن حرفين.",
      passwordTooShort: "يجب ألا تقل كلمة المرور عن 6 أحرف.",
      passwordsDoNotMatch: "كلمتا المرور غير متطابقتين.",
      accountType: "نوع الحساب",
      hostDescription: "أنشئ الاختبارات وشغّل المسابقات المباشرة",
      studentDescription: "انضم إلى المسابقات وتابع تقدّمك",
      adminDescription: "تحكم كامل في كل مستخدم واختبار ومسابقة",
      adminKey: "مفتاح تسجيل المسؤول",
      adminKeyPlaceholder: "الصق المفتاح الذي أُعطيته",
      adminKeyHint: "مطلوب للتسجيل كمسؤول — اطلب واحدًا من مسؤول موجود.",
      adminKeyRequired: "أدخل مفتاح تسجيل المسؤول.",
      confirmPassword: "تأكيد كلمة المرور",
      minCharsHint: "6 أحرف على الأقل.",
      codeIntro: "نرسل إليك رمزًا — بالبريد أو برسالة نصية. لا كلمة مرور تحفظها.",
      sendCode: "أرسل لي رمزًا",
      codeSentTo: "أرسلنا رمزًا إلى {email}.",
      codeLabel: "الرمز الذي وصلك",
      codeWrong: "الرمز غير صحيح أو انتهت صلاحيته. راجع الرسالة أو اطلب رمزًا جديدًا.",
      codeExpired: "انتهت صلاحية الرمز. اطلب رمزًا جديدًا.",
      codeTooShort: "أدخل الرمز كاملًا كما وصلك.",
      resendCode: "أعد الإرسال",
      resendIn: "أعد الإرسال بعد {seconds} ث",
      changeEmail: "استخدم وسيلة أخرى",
      emailInvalid: "أدخل بريدًا إلكترونيًا صحيحًا.",
      noAccountForEmail: "لا يوجد حساب بهذا البريد أو الرقم. أنشئ حسابًا أولًا.",
      tooManyCodes: "طلبت رموزًا كثيرة. انتظر دقيقة ثم حاول مجددًا.",
      accountMadeButNoCode: "أُنشئ حسابك، لكن تعذّر إرسال الرمز. اطلبه من صفحة تسجيل الدخول.",
      emailOrPhone: "البريد الإلكتروني أو رقم الهاتف",
      emailOrPhonePlaceholder: "you@example.com  أو  ‎+216 22 345 678",
      emailOrPhoneHint: "بالبريد يصلك الرمز في بريدك، وبالرقم يصلك برسالة نصية.",
      contactInvalid: "أدخل بريدًا إلكترونيًا، أو رقم هاتف مع رمز الدولة.",
      smsUnsupported: "لا يمكننا إرسال رسالة نصية إلى هذا الرقم. استخدم بريدًا إلكترونيًا.",
      noOtherChannel: "لا توجد وسيلة أخرى للوصول إلى هذا الرقم. استخدم بريدًا إلكترونيًا.",
      sentByChannel: "أُرسل عبر {channel}.",
      tryAnotherWay: "لم يصلك شيء؟ جرّب وسيلة أخرى",
      smsNotConfigured: "الدخول برسالة نصية غير مُعدّ بعد. استخدم بريدًا إلكترونيًا.",
      smsUnavailable: "تعذّر إرسال الرموز برسالة نصية حاليًا. استخدم بريدًا إلكترونيًا.",
      smsFailed: "تعذّر إرسال الرمز. حاول مجددًا، أو استخدم بريدًا إلكترونيًا.",
      checkFailed: "تعذّر التحقق من هذا الرمز. اطلب رمزًا جديدًا.",
      tooManyAttempts: "أخطأت في الرمز مرات كثيرة. اطلب رمزًا جديدًا.",
    },
    join: {
      nameTaken: "هذا الاسم مستخدم بالفعل في هذه المسابقة. اختر اسمًا آخر.",
      lockedError: "أقفل المعلم هذه المسابقة، فلا يمكن لأحد الانضمام الآن.",
      rejoinTitle: "أنت بالفعل في مسابقة",
      rejoinDesc: "{name} في المسابقة {code}. عد إليها أو ابدأ من جديد.",
      rejoin: "العودة إلى مسابقتي",
      startFresh: "ابدأ من جديد",
      title: "انضم إلى مسابقة",
      subtitle: "أدخل رمز مسابقة أو صف شاركه معلمك، ثم اختر اسمًا مستعارًا.",
      nickname: "اسمك المستعار",
      nicknamePlaceholder: "مثال: أحمد",
      gameCode: "الرمز",
      gameCodePlaceholder: "مثال: A1B2C3D4",
      joinButton: "انضم إلى المسابقة",
      nickError: "يجب أن يكون الاسم بين 2 و50 حرفًا.",
      codeError: "أدخل رمز المسابقة الذي يعرضه المضيف.",
      notOpenError: "هذه المسابقة غير مفتوحة للانضمام بعد. تحقق من الرمز مع المضيف.",
      draftError:
        "لم يُطلق هذا الاختبار بعد. اطلب من المضيف إطلاقه من مكتبة الاختبارات — لا يمكن الانضمام إلى المسودات.",
      classCodeError:
        "هذا رمز صف، وليس رمز مسابقة. انضم إلى الصف من صفحة صفوفك — المسابقات المباشرة تستخدم الرمز الظاهر في غرفة تحكم المضيف.",
      classCodeNeedsLogin:
        "أكواد الصفوف تتطلب تسجيل الدخول. سجّل الدخول ثم انضم إلى الصف من صفحة صفوفك.",
      genericErrorTitle: "تعذّر الانضمام إلى المسابقة",
      genericErrorDesc: "تحقق من اتصالك وحاول مجددًا.",
    },
    student: {
      trend: "تطورك",
      trendSub: "نتائجك من أقدم مسابقة إلى الأحدث.",
      progress: "تقدّمي",
      progressSub: "سجل مسابقاتك مرتبط بحسابك — سجّل الدخول قبل الانضمام لتتبعه.",
      gamesPlayed: "المسابقات التي شاركت فيها",
      joined: "انضممت",
      avgScore: "متوسط النقاط",
      avgAccuracy: "متوسط الدقة",
      bestScore: "أفضل نتيجة",
      recentGames: "المسابقات الأخيرة",
      noGamesTitle: "لا توجد مسابقات بعد",
      noGamesDesc: "انضم إلى مسابقة مباشرة بالرمز الذي يعرضه مضيفك — ستظهر النتائج هنا.",
      historyTitle: "سجل المسابقات",
      historySub: "كل مسابقة انضممت إليها أثناء تسجيل الدخول، مع نتيجتك ودقتك.",
      correctOf: "إجابات صحيحة",
      pts: "نقطة",
      myClasses: "صفوفي",
      myClassesSub: "انضم بالرمز الذي يشاركه معلمك لتبقى ضمن مجموعته.",
      joinClass: "الانضمام إلى صف",
      joinClassSub: "أدخل رمز الصف الذي شاركه معلمك.",
      classCode: "رمز الصف",
      classCodePlaceholder: "مثال: SBJ2S9YY",
      joinClassButton: "انضم إلى الصف",
      classCodeError: "أدخل رمز الصف الذي شاركه معلمك.",
      classNotOpen: "رمز الصف هذا غير مفتوح الآن. تحقق مع معلمك.",
      classJoined: "تم الانضمام إلى الصف",
      classLeft: "غادرت الصف",
      noClassesTitle: "لا توجد صفوف بعد",
      noClassesDesc: "اسأل معلمك عن رمز الصف وأدخله أعلاه.",
      members: "عضوًا",
      signInToTrack: "سجّل الدخول لتتبع تقدّمك",
      classJoinedDesc: "يمكنك الآن رؤية صفك هنا.",
      classJoinFailed: "تعذّر الانضمام إلى الصف",
      classLeftDesc: "أُزيل {name} من قائمتك.",
      classLeaveFailed: "تعذّر مغادرة الصف",
      classesLoadFailed: "تعذّر تحميل صفوفك",
    },
    admin: {
      overviewTitle: "الإدارة",
      overviewSub: "تحكم كامل في كل مستخدم واختبار ومسابقة.",
      usersTitle: "المستخدمون",
      usersSub: "كل حساب على المنصة.",
      competitionsTitle: "المسابقات",
      competitionsSub: "كل مسابقة، من جميع المضيفين.",
      roleLabel: "الدور",
      roleStudent: "طالب",
      roleHost: "مضيف",
      roleAdmin: "مسؤول",
      roleUpdated: "تم تحديث الدور",
      roleUpdateFailed: "تعذّر تحديث الدور",
      deleteUser: "حذف الحساب",
      deleteUserTitle: "حذف هذا الحساب؟",
      deleteUserDesc: "سيُحذف نهائيًا مع كل ما يخصه. لا يمكن التراجع عن هذا.",
      userDeleted: "تم حذف الحساب",
      userDeleteFailed: "تعذّر حذف الحساب",
      deleteCompetitionTitle: "حذف هذه المسابقة؟",
      deleteCompetitionDesc: "ستُحذف نهائيًا مع أسئلتها وإجاباتها. لا يمكن التراجع عن هذا.",
      competitionDeleted: "تم حذف المسابقة",
      competitionDeleteFailed: "تعذّر حذف المسابقة",
      loadFailed: "تعذّر تحميل القائمة. أعد المحاولة.",
    },
    host: {
      classCanJoin: "يمكن لأعضاء الصف الدخول من لوحتهم",
      allowLateJoin: "السماح بالانضمام بعد البدء",
      correctSoFar: "صحيحة",
      incorrectSoFar: "غير صحيحة",
      playAgain: "شارك مرة أخرى",
      playAgainReady: "المسابقة الجديدة جاهزة",
      playAgainFailed: "تعذر بدء مسابقة جديدة",
      share: "مشاركة",
      shareText: "انضم إلى مسابقتي بالرمز {code}",
      addTime: "+30 ث",
      lockJoining: "قفل الانضمام",
      sortBy: "ترتيب حسب",
      sortRecent: "الأحدث تحديثًا",
      sortName: "الاسم",
      sortQuestions: "الأكثر أسئلة",
      answerSpread: "توزيع الإجابات",
      showSpread: "إظهار",
      hideSpread: "إخفاء",
      autoAdvance: "الانتقال التلقائي",
      copyLink: "نسخ الرابط",
      scanToJoin: "امسح الرمز للانضمام",
      exportCsv: "تصدير CSV",
      exportFailed: "فشل التصدير",
      csvPlayer: "اللاعب",
      csvQuestionNo: "رقم السؤال",
      csvQuestion: "السؤال",
      csvAnswer: "الإجابة",
      csvCorrect: "صحيحة",
      csvResponseMs: "زمن الإجابة (مللي ثانية)",
      allTimeHeading: "الإجمالي",
      gamesLiveNow: "{count} مباشرة الآن",
      statStudentsReached: "عدد الطلاب",
      acrossPlayers: "{count} مشاركة إجمالًا",
      allGames: "في كل المسابقات",
      quizzesDrafted: "{count} مسودة",
      myGamesTitle: "المسابقات المباشرة",
      myGamesSub: "المسابقات التي أطلقتها. افتح غرفة التحكم لإدارة جولة.",
      browseQuizzes: "تصفح الاختبارات",
      cancel: "إلغاء",
      cancelledToastTitle: "أُلغيَت المسابقة",
      cancelledToastDesc: "تم إيقاف",
      cancelFailed: "تعذّر الإلغاء",
      loadFailedTitle: "تعذّر تحميل مسابقاتك",
      loadFailedDesc: "تحقق من اتصالك وحاول مجددًا.",
      openControlRoom: "فتح غرفة التحكم",
      noGamesTitle: "لا توجد مسابقات بعد",
      noGamesDesc:
        "اختر اختبارًا من مكتبتك وأطلقه — ينضم الطلاب عبر رمز الغرفة.",
      librarySub: "ابنِ اختباراتك ونظّمها، ثم أطلقها كمسابقات مباشرة.",
      searchPlaceholder: "ابحث بالاسم أو الرمز أو التصنيف…",
      searchLabel: "البحث في الاختبارات",
      of: "من",
      noQuizzesYet: "لا توجد اختبارات بعد",
      noQuizzesDesc:
        "أنشئ اختبارك الأول — الأسئلة والنقاط ومراجع القرآن كلها قابلة للتحرير.",
      noMatches: "لا نتائج مطابقة",
      noMatchesDesc: "جرّب كلمة بحث مختلفة.",
      createQuiz: "إنشاء اختبار",
      duplicatedTitle: "تم تكرار الاختبار",
      duplicatedDesc: "افتح النسخة لتحريرها.",
      duplicateFailed: "فشل التكرار",
      deletedTitle: "تم حذف الاختبار",
      deletedDesc: "أُزيل «{name}».",
      deleteFailed: "فشل الحذف",
      archivedTitle: "تمت الأرشفة",
      archivedDesc: "نُقل خارج المكتبة النشطة.",
      archiveFailed: "فشلت الأرشفة",
      deleteTitle: "حذف هذا الاختبار؟",
      deleteDesc: "سيُحذف «{name}» نهائيًا مع أسئلته البالغة {count} سؤالًا.",
      deletePermanently: "حذف نهائي",
      lobbyOpen: "القاعة مفتوحة",
      setupTitle: "إعداد المسابقة",
      setupTagline:
        "حدّد عنوان المسابقة الذي يراه الطلاب، والتعليمات الاختيارية، والمدة المتاحة لكل سؤال.",
      setupGameTitle: "عنوان المسابقة",
      setupGameTitlePlaceholder: "مثال: جولة الليل القرآنية 2",
      setupInstructions: "التعليمات (اختياري)",
      setupInstructionsPlaceholder: "تُعرض للطلاب أثناء انتظارهم في القاعة…",
      setupMinutesPerQuestion: "الوقت لكل سؤال (دقائق)",
      setupMinutesHint: "تُقصّ الأسئلة الأطول من هذا الحد إلى الحد نفسه عند بدء السؤال.",
      setupSave: "حفظ الإعداد",
      setupSaved: "تم حفظ الإعداد",
      setupSaveFailed: "تعذّر حفظ الإعداد",
      gameNotFound: "المسابقة غير موجودة. تحقق من رمز الغرفة.",
      couldNotOpen: "تعذّر فتح هذه المسابقة.",
      backToGames: "العودة إلى المسابقات",
      couldNotCopy: "تعذّر نسخ الرمز.",
      actionFailed: "فشل الإجراء",
      gameDeleted: "تم حذف المسابقة",
      gameDeletedDesc: "أُزيلت جميع الجولات واللاعبين.",
      pause: "إيقاف مؤقت",
      startGame: "بدء المسابقة",
      editQuestions: "تعديل الأسئلة",
      addQuestion: "إضافة سؤال",
      addQuestionDesc: "تُضاف الأسئلة الجديدة بعد الأسئلة الحالية ويمكن بدء أي منها في أي وقت.",
      questionAdded: "تمت إضافة السؤال",
      endQuestionNow: "إنهاء السؤال الآن",
      nextQuestion: "السؤال التالي",
      finishGame: "إنهاء المسابقة",
      resumeGame: "استئناف المسابقة",
      questionOfCtrl: "السؤال {position} من {total}",
      remaining: "متبقٍ",
      closed: "مغلق",
      answered: "أجابوا",
      correctAnswer: "الإجابة الصحيحة",
      noActiveQuestion: "لا يوجد سؤال نشط.",
      questionDeck: "قائمة الأسئلة",
      active: "نشط",
      done: "منتهٍ",
      upcoming: "قادم",
      results: "النتائج",
      place: "المركز",
      perQuestionBreakdown: "تفصيل كل سؤال",
      noQuestionData: "لا توجد بيانات عن الأسئلة.",
      shareCode: "شارك الرمز",
      studentsGoJoin: "يذهب الطلاب إلى {path} ويدخلون هذا الرمز للانضمام إلى القاعة.",
      playersHeading: "اللاعبون",
      waitingPlayers: "بانتظار انضمام اللاعبين…",
      online: "متصل",
      away: "غير متصل",
      noScoresYet: "لا توجد نقاط بعد.",
      dangerZone: "منطقة الخطر",
      cancelGame: "إلغاء المسابقة",
      deleteGame: "حذف المسابقة",
      cancelTitle: "إلغاء هذه المسابقة؟",
      cancelDesc: "تُغلق الغرفة ولن يتمكن اللاعبون من الإجابة. تبقى النتائج متاحة.",
      keepPlaying: "مواصلة المشاركة",
      deleteGameTitle: "حذف هذه المسابقة؟",
      deleteGameDesc: "سيُحذف نهائيًا المسابقة وجولاتها وكل سجلات اللاعبين.",
      keepIt: "الإبقاء عليها",
      copied: "تم النسخ!",
      classesTitle: "الصفوف",
      classesSub: "جمّع طلابك، شارك رمزًا، واربط الاختبارات بصف.",
      createClass: "إنشاء صف",
      createClassSub: "شارك الرمز مع طلابك.",
      className: "اسم الصف",
      classNamePlaceholder: "مثال: مجموعة القرآن أ",
      classDescription: "الوصف (اختياري)",
      classDescriptionPlaceholder: "مثال: الحصة الثانية، مبتدئون",
      createdToast: "تم إنشاء الصف",
      createdToastDesc: "شارك الرمز مع طلابك.",
      createFailed: "تعذّر إنشاء الصف",
      membersLabel: "الأعضاء",
      archiveLabel: "أرشفة",
      archivedToast: "تمت أرشفة الصف",
      archivedToastDesc: "لا يمكن للطلاب الانضمام إليه بعد الآن.",
      archiveTitle: "أرشفة هذا الصف؟",
      archiveDesc: "لن يقبل أعضاءً جددًا. التعيينات القائمة تبقى.",
      keepClass: "إبقاء الصف",
      archivedBadge: "مؤرشَف",
      noMembers: "لا أعضاء بعد",
      noMembersDesc: "شارك رمز الصف ليلتحق الطلاب.",
      memberRemoved: "تمت إزالة العضو",
      removeMemberFailed: "تعذّرت إزالة العضو",
      noClassesYet: "لا توجد صفوف بعد",
      copyFailedDesc: "حدّد الرمز يدويًا.",
      membersLoadFailed: "تعذّر تحميل الأعضاء",
      createFailedDesc: "تحقق من الاسم وحاول مجددًا.",
      memberWord: "أعضاء",
      gameWord: "مسابقات",
      copyClassCode: "نسخ رمز الصف",
      membersJoinHint: "ينضم الطلاب من صفحة الصفوف لديهم.",
      joinedOn: "انضم",
      removeMember: "إزالة",
      noClassesYetDesc: "أنشئ صفك الأول وشارك رمزه مع الطلاب.",
      copyFailed: "تعذّر النسخ",
      codeCopied: "تم نسخ الرمز",
      codeCopiedDesc: "رمز الصف",
      analyticsTitle: "التحليلات",
      analyticsSub: "النقاط والدقة وزمن الاستجابة والأسئلة التي أخطأ فيها لاعبوك.",
      manageGames: "إدارة المسابقات",
      gamesLaunched: "المسابقات المطلقة",
      averageScore: "متوسط النتيجة",
      averageAccuracy: "متوسط الدقة",
      avgResponseTime: "متوسط زمن الاستجابة",
      mostMissed: "الأسئلة الأكثر إجابة خاطئة",
      mostMissedSub: "الأسئلة التي أخطأ فيها اللاعبون في مسابقة",
      answersCount: "إجابة",
      noAnswers: "لا إجابات بعد",
      nothingMissed: "لم يُخطئ أحد",
      nothingMissedDesc: "لا توجد إجابات خاطئة مسجلة لهذه المسابقة.",
      wrongAnswers: "إجابة خاطئة",
      correctPct: "% صحيحة",
      questions: "أسئلة",
      answers: "إجابات",
      gameBadgeFinished: "منتهية",
      gameBadgeCancelled: "ملغاة",
      gameBadgeRunning: "جارية",
      gameBadgeWaiting: "بانتظار اللاعبين",
      gameBadgePaused: "متوقفة مؤقتًا",
      loadAnalyticsFailed: "تعذّر تحميل التحليلات",
      loadGamesFailed: "تعذّر تحميل مسابقاتك",
      statQuestions: "أسئلة",
      statAnswers: "إجابات",
      inGame: "في مسابقة {code}",
      pickAnotherGame: "حاول مجددًا، أو اختر مسابقة أخرى.",
    },
    editor: {
      savedAt: "حُفظ {time}",
      filterQuestions: "تصفية الأسئلة…",
      duplicateQuestion: "تكرار هذا السؤال",
      typeOrdering: "الترتيب",
      hintLabel: "تلميح (اختياري)",
      hintHelp: "يمكن للطلاب إظهاره أثناء فتح السؤال.",
      showHint: "أظهر تلميحًا",
      printSheet: "طباعة ورقة",
      answerKey: "مفتاح الإجابات",
      noAnswerSet: "— لا توجد إجابة —",
      printedFrom: "طُبع من منصة مسابقات القرآن",
      typePageWords: "الكلمات المخفية في صفحة",
      exportJson: "تصدير JSON",
      exportDone: "تم تصدير الاختبار",
      importJson: "استيراد JSON",
      importDone: "تم استيراد الاختبار",
      importDoneDesc: "راجع الأسئلة ثم ابدأ المسابقة عندما تكون جاهزًا.",
      importFailed: "فشل الاستيراد",
      importInvalidJson: "هذا الملف ليس ملف اختبار JSON صالحًا.",
      importTooBig: "حجم الملف كبير جدًا (الحد الأقصى 2 ميغابايت).",
      questionLocked: "هذا السؤال بدأ بالفعل ولا يمكن تعديله.",
      alreadyPlayed: "تم عرضه",
      settings: "الإعدادات",
      quizName: "اسم الاختبار",
      language: "اللغة",
      category: "التصنيف",
      categoryPlaceholder: "مثال: حفظ",
      difficulty: "الصعوبة",
      difficultyAny: "أي",
      difficultyEasy: "سهل",
      difficultyMedium: "متوسط",
      difficultyHard: "صعب",
      defaultPoints: "النقاط الافتراضية للسؤال",
      defaultNegativePoints: "النقاط السالبة الافتراضية (إجابة خاطئة)",
      classOptional: "الصف (اختياري)",
      noClass: "بدون صف",
      description: "الوصف (يظهر للاعبين)",
      descriptionPlaceholder: "مثال: سورة البقرة، الآيات 1–10",
      speedBonus: "منح مكافأة سرعة للإجابات الصحيحة السريعة",
      questions: "الأسئلة",
      addQuestion: "إضافة سؤال",
      noDescription: "لا يوجد وصف",
      editLabel: "تحرير",
      openLabel: "فتح",
      duplicateAction: "تكرار",
      createQuiz: "إنشاء اختبار",
      createSub: "ستضيف الأسئلة بعد ذلك مباشرة — يبقى الاختبار مسودة حتى تطلقه مباشرًا.",
      quizNamePlaceholder: "مثال: أساسيات سورة البقرة",
      createAndAdd: "إنشاء وإضافة الأسئلة",
      quizCreated: "تم إنشاء الاختبار",
      quizCreatedDesc: "أضف أسئلتك الآن.",
      createFailed: "تعذّر إنشاء الاختبار.",
      quizNameRequired: "أعطِ اختبارك اسمًا أولًا.",
      saveChanges: "حفظ التغييرات",
      savedToastTitle: "تم حفظ الاختبار",
      savedToastDesc: "خُزِّنت جميع التغييرات.",
      saveFailed: "حدث خطأ أثناء الحفظ.",
      launchGame: "إطلاق مسابقة مباشرة",
      launchToastTitle: "أُطلقت المسابقة",
      launchToastDesc: "قاعة الانتظار مفتوحة — شارك الرمز.",
      launchFailed: "حاول مجددًا.",
      launchDialogTitle: "إطلاق هذا الاختبار؟",
      launchDialogDesc: "رمز القاعة هو",
      launchDialogKeep: "مواصلة التحرير",
      launchDialogConfirm: "إطلاق المسابقة",
      deleteQuestion: "حذف السؤال",
      preview: "معاينة",
      backToLibrary: "العودة إلى المكتبة",
      exitEditing: "الخروج من التحرير",
      exitConfirmTitle: "مغادرة المحرر؟",
      exitConfirmDesc: "ستُفقد التغييرات غير المحفوظة.",
      exitConfirmKeep: "البقاء هنا",
      exitConfirmLeave: "مغادرة",
      typeLabel: "النوع",
      typeMcq: "اختيار من متعدد",
      typeTrueFalse: "صواب / خطأ",
      typeText: "نص قصير",
      typeNumber: "رقم",
      typeAudio: "صوتي",
      questionText: "نص السؤال",
      questionTextPlaceholder: "اكتب السؤال…",
      choices: "الخيارات",
      choiceLabel: "خيار",
      addChoice: "إضافة خيار",
      correctAnswer: "الإجابة الصحيحة",
      markCorrect: "تحديد كصحيح",
      duration: "المدة (بالثواني)",
      seconds: "ث",
      pointsPerQuestion: "النقاط",
      default: "الافتراضي",
      negativePoints: "النقاط السالبة",
      explanation: "التفسير (اختياري)",
      explanationPlaceholder: "يظهر بعد انتهاء السؤال…",
      surahRef: "مرجع قرآني",
      surah: "السورة",
      ayah: "الآية",
      page: "الصفحة",
      juz: "الجزء",
      hizb: "الحزب",
      missingText: "نص السؤال مطلوب.",
      missingChoices: "كل خيار يحتاج نصًا.",
      missingTwoChoices: "أضف خيارين على الأقل.",
      missingCorrectMarker: "حدد خيارًا واحدًا كصحيح.",
      missingCorrectText: "أدخل نص الإجابة الصحيحة.",
      missingDuration: "يجب أن تكون المدة 5 ثوانٍ على الأقل.",
      missingPoints: "لا يمكن أن تكون النقاط سالبة.",
      errorSummary: "بعض الأسئلة غير مكتملة؛ صحّحها قبل الحفظ.",
      notDraftLocked: "التحرير مقفل بعد إطلاق المسابقة.",
      moveUp: "تحريك لأعلى",
      moveDown: "تحريك لأسفل",
      loadTitle: "تعذّر فتح هذا الاختبار",
      loadNotFound: "الاختبار غير موجود أو ليس لديك صلاحية الوصول.",
      loadFailed: "تعذّر تحميل الاختبار.",
      backToQuizzes: "العودة إلى اختباراتي",
      checkErrors: "تحقق من الأخطاء",
      checkErrorsDesc: "بعض الأسئلة غير مكتملة؛ أصلحها قبل الحفظ.",
      saveFailedTitle: "فشل الحفظ",
      launchFailedTitle: "فشل الإطلاق",
      untitledQuiz: "اختبار بدون عنوان",
      untitledQuestion: "سؤال بدون عنوان",
      unsaved: "غير محفوظ",
      discard: "تجاهل",
      addQuestionsFirst: "أضف الأسئلة أولًا",
      launched: "تم إطلاقه",
      backToEditing: "العودة للتحرير",
      incomplete: "غير مكتمل",
      launchDialogLocked: "يمكن للاعبين الانضمام فور فتح الصالة — سيتم قفل التحرير.",
      onlyOneCorrect: "إجابة واحدة فقط يمكن أن تكون الصحيحة.",
      chooseCorrect: "اختر الإجابة الصحيحة.",
      optionsMark: "الخيارات — علّم الصحيح",
      markOptionAria: "تحديد الخيار {n} كإجابة صحيحة",
      removeOptionAria: "إزالة الخيار {n}",
      optionWord: "خيار",
      addOption: "إضافة خيار",
      trueOption: "صحيح",
      falseOption: "خطأ",
      correctNumeric: "الإجابة الرقمية الصحيحة",
      correctTextHint: "الإجابة الصحيحة (مطابقة نصية دقيقة مع تجاهل حالة الأحرف)",
      expected: "المتوقعة",
    },
    game: {
      fullscreen: "ملء الشاشة",
      exitFullscreen: "إنهاء ملء الشاشة",
      streakTitle: "إجابات صحيحة متتالية",
      answerRecorded: "تم تسجيل إجابتك: {points} نقطة.",
      lobbyTitle: "قاعة الانتظار",
      lobbySub: "جهّز جهازك — سيبدأ المضيف الأسئلة قريبًا.",
      waitingForPlayers: "بانتظار اللاعبين",
      startingSoon: "لم تبدأ المسابقة بعد.",
      gameEnded: "انتهت المسابقة.",
      leaveLobby: "مغادرة القاعة",
      playerCount: "لاعبًا انضموا",
      questionTitle: "سؤال",
      waitingForQuestion: "لم يرسل المضيف سؤالًا بعد.",
      pausedOverlay: "متوقفة مؤقتًا",
      pausedOverlayDesc: "أوقف المضيف المسابقة — انتظر قليلًا.",
      submitAnswer: "إرسال الإجابة",
      answerLocked: "الإجابة مقفلة",
      skipped: "تخطّيت",
      correct: "صحيحة",
      wrong: "خاطئة",
      feedback: "النتيجة",
      youAnswered: "أجبت بـ:",
      correctAnswerIs: "الإجابة الصحيحة:",
      explanation: "التفسير",
      next: "التالي",
      resultTitle: "نتائج المسابقة",
      yourScore: "نتيجتك",
      yourRank: "ترتيبك",
      ofPlayers: "من",
      leaderboard: "لوحة الصدارة",
      podium: "المنصة",
      yourAnswers: "إجاباتك",
      timeUp: "انتهى الوقت!",
      answerNow: "أجب الآن",
      kbdChoice: "اضغط 1–4 للاختيار",
      networkError: "انقطع الاتصال — إعادة الاتصال…",
      retryJoin: "إعادة الاتصال",
      sessionExpired: "انتهت جلستك. يرجى الانضمام مجددًا.",
      backToLobby: "العودة إلى القاعة",
      answeredCount: "أجاب",
      playersAnswered: "لاعب أجابوا",
      connecting: "جارٍ الاتصال بالمسابقة…",
      connectError: "تعذّر الوصول إلى المسابقة. تحقق من اتصالك.",
      notAvailableTitle: "المسابقة غير متاحة",
      notAvailableDesc: "أغلق المضيف هذه المسابقة أو لم تعد قابلة للانضمام.",
      you: "أنت",
      questionOf: "السؤال {position} من {total}",
      scoreLabel: "النقاط: {total}",
      pickAnswerError: "اختر إجابة قبل الإرسال.",
      questionClosedError: "أُغلق هذا السؤال للتو. انتظر حتى يواصل المضيف.",
      sendAnswerFailed: "تعذّر إرسال إجابتك",
      gettingReady: "جارٍ التحضير…",
      hostAboutToStart: "المضيف على وشك بدء سؤال.",
      answerSubmitted: "تم إرسال الإجابة",
      waitingTimer: "بانتظار انتهاء المؤقت…",
      wrongAnswer: "إجابة خاطئة",
      noAnswerSubmitted: "لم تُرسل إجابة",
      youPickedThis: "اخترت هذا",
      why: "السبب",
      revealingAnswer: "جارٍ كشف الإجابة…",
      loadingResults: "جارٍ تحميل النتائج…",
      gameEndedTitle: "انتهت المسابقة",
      leaveGame: "مغادرة المسابقة",
      answerChoicesLabel: "خيارات الإجابة",
      typeNumber: "اكتب رقمًا…",
      typeAnswer: "اكتب إجابتك…",
      gameStillRunning: "المسابقة ما زالت جارية",
      resultsAfterFinish: "تظهر النتائج بمجرد إنهاء المضيف لمسابقة.",
      backToGame: "العودة إلى المسابقة",
      gameFinishedTitle: "انتهت المسابقة!",
      yourResultIntro: "{name}، هذه نتيجتك.",
      returnHome: "العودة إلى الرئيسية",
      firstPlace: "الأول",
      secondPlace: "الثاني",
      thirdPlace: "الثالث",
    },
    pw: {
      submitPartial: "إرسال {placed} من {total}",
      autosaveHint: "يُحفظ عملك أولًا بأول — ويُحتسب حتى لو انتهى الوقت.",
      autoFill: "تعبئة من بيانات الصفحة",
      hideCount: "عدد الكلمات المخفية",
      annotatedWords: "{count} كلمة محددة في هذه الصفحة",
      noAnnotations: "لا توجد بيانات كلمات لهذه الصفحة — حدد المربعات يدويًا.",
      imageNotReady: "انتظر تحميل صورة الصفحة ثم أعد المحاولة.",
      autoFillFailed: "تعذر قراءة بيانات الكلمات لهذه الصفحة.",
      typeForBox: "اكتب الكلمة للمربع {n}",
      typeNext: "اكتب كلمة لوضعها",
      typePlaceholder: "اكتب الكلمة…",
      place: "ضع",
      noSuchWord: "لا توجد كلمة متبقية تطابق ما كتبته.",
      pageLabel: "صفحة المصحف",
      pageOption: "صفحة {page}",
      drawHint: "اسحب على الصفحة لتحديد كل كلمة تريد إخفاءها",
      boxCount: "{count} مربع",
      wordLabel: "الكلمة في كل مربع (بالترتيب)",
      wordPlaceholder: "الكلمة المخفية…",
      removeBox: "حذف المربع {n}",
      incomplete: "كل مربع يحتاج كلمته.",
      needBox: "حدد كلمة واحدة على الأقل في الصفحة.",
      progress: "{placed} / {total} تم وضعها",
      playTitle: "أعد كل كلمة إلى موضعها",
      playHint: "اضغط على الكلمة ثم على المربع الفارغ، أو اسحبها إليه. اضغط على مربع ممتلئ لإفراغه.",
      boxAria: "مربع فارغ {n}",
      wordsAria: "الكلمات المطلوب وضعها",
    },
    theme: {
      switchToDark: "التبديل إلى الوضع الداكن",
      switchToLight: "التبديل إلى الوضع الفاتح",
    },
    ord: {
      hint: "اضغط الكلمات بالترتيب الصحيح.",
      progress: "{placed} من {total}",
      emptySlot: "فارغ",
      poolAria: "الكلمات المطلوب ترتيبها",
      editorHint: "أدخل الأجزاء بالترتيب الصحيح — تُخلط للطلاب.",
      itemCount: "{count} عناصر",
      itemPlaceholder: "جزء…",
      addItem: "إضافة عنصر",
      needTwo: "سؤال الترتيب يحتاج عنصرين على الأقل.",
    },
    audioQ: {
      play: "تشغيل",
      pause: "إيقاف",
      playAgain: "تشغيل مرة أخرى",
      restart: "من البداية",
      notPlayed: "لم يُشغَّل بعد",
      playCount: "شُغِّل {count} مرة",
      failed: "تعذّر تحميل الصوت.",
      urlLabel: "رابط الصوت",
      urlHint: "رابط مباشر لملف mp3 أو ogg.",
      needUrl: "سؤال الصوت يحتاج رابطًا.",
      fromQuran: "اختر مقطعًا من صفحة",
      preview: "استمع إليه",
      useRange: "استخدم هذا المقطع",
      rangeSet: "صفحة {page}: {passage}",
      random: "مقطع عشوائي",
      askLabel: "اسأل عن",
      askSurah: "السورة",
      askAyah: "الآية",
      askHizb: "الحزب",
      answerIs: "الإجابة الصحيحة: {answer}",
      hizbBoundary: "يبدأ ربع جديد في هذه الصفحة، فجزء منها يتبع الحزب التالي.",
      promptSurah: "استمع: من أي سورة هذا المقطع؟",
      promptAyah: "استمع: من أي آية يبدأ هذا المقطع؟",
      promptHizb: "استمع: في أي حزب هذا المقطع؟",
    },
    presenter: {
      open: "وضع العرض",
      exit: "خروج (Esc)",
      joinAt: "انضم بالرمز",
      answered: "أجاب {count} من {total}",
    },
    sound: {
      turnOn: "تشغيل الأصوات",
      turnOff: "إيقاف الأصوات",
    },
    accent: {
      aria: "لون الواجهة",
    },
    call: {
      connecting: "جارٍ الاتصال…",
      unmute: "إلغاء الكتم",
      stopCam: "إيقاف الكاميرا",
      startCam: "تشغيل الكاميرا",
      everyone: "الجميع:",
      muteAll: "كتم الجميع",
      unmuteAll: "إلغاء كتم الجميع",
      camsOff: "إيقاف الكاميرات",
      camsOn: "تشغيل الكاميرات",
      byHost_micOff: "قام المعلم بكتم الميكروفون. اضغط على الميكروفون للتحدث مجددًا.",
      byHost_micOn: "قام المعلم بتشغيل الميكروفون.",
      byHost_camOff: "قام المعلم بإيقاف الكاميرا.",
      byHost_camOn: "قام المعلم بتشغيل الكاميرا.",
      relayNone: "لا يوجد خادم ترحيل — قد تفشل المكالمات على الشبكات المقيدة.",
      relayPublic: "يُستخدم الترحيل العام المجاني — مناسب للتجربة، واضبط خادمك للصف.",
      relayConfigured: "الترحيل: {source}",
      title: "الصوت والكاميرا",
      blurb: "تحدث مع الصف أثناء المشاركة.",
      join: "انضم للمكالمة",
      leave: "مغادرة",
      live: "{count} في المكالمة",
      micOn: "الميكروفون يعمل",
      micOff: "الميكروفون مغلق",
      camOn: "الكاميرا تعمل",
      camOff: "الكاميرا مغلقة",
      noCamera: "لا توجد كاميرا على هذا الجهاز",
      you: "أنت",
      student: "طالب",
      mute: "كتم",
      mutedByHost: "قام المعلم بكتم الميكروفون. اضغط على الميكروفون للتحدث مجددًا.",
      waiting: "في انتظار انضمام الآخرين…",
      denied: "تم رفض إذن الميكروفون. اسمح به في المتصفح للانضمام.",
      failed: "تعذّر بدء المكالمة على هذا الجهاز.",
      roomTitle: "غرفة الصوت والكاميرا",
      roomHint: "لن يرى الطلاب المكالمة إلا بعد فتحها.",
      open: "مفتوحة",
      closed: "مغلقة",
    },
    teams: {
      label: "الفرق:",
      count: "عدد الفرق",
      none: "بدون فرق",
      nTeams: "{n} فرق",
      shuffle: "توزيع عشوائي",
      standings: "ترتيب الفرق",
      setFor: "فريق {name}",
      points: "نقاط",
      remove: "إزالة",
      awardTitle: "منح نقاط",
      awardDesc: "امنح أو اخصم نقاطًا من {name}.",
      award: "منح",
      awardHint: "استخدم رقمًا سالبًا للخصم.",
      removeTitle: "إزالة اللاعب؟",
      removeDesc: "سيتم إخراج {name} من هذه المسابقة.",
    },
    picker: {
      open: "اختيار عشوائي",
      title: "طالب عشوائي",
      desc: "يختار لاعبًا واحدًا عشوائيًا.",
      pick: "اختر",
      again: "مرة أخرى",
      ready: "جاهز",
    },
    palette: {
      title: "تنقل سريع",
      placeholder: "انتقل إلى… (Ctrl+K)",
      noMatch: "لا نتائج.",
    },
    net: {
      offline: "أنت غير متصل — ستُرسل الإجابات عند عودة الاتصال.",
      back: "عاد الاتصال",
    },
    shortcuts: {
      title: "اختصارات لوحة المفاتيح",
      desc: "أثناء التركيز على غرفة التحكم.",
      next: "بدء / السؤال التالي",
      pause: "إيقاف مؤقت أو استئناف",
      presenter: "وضع العرض",
      spread: "عرض توزيع الإجابات",
      palette: "تنقل سريع",
      help: "هذه المساعدة",
      escape: "إغلاق المفتوح",
    },
    avatar: {
      label: "اختر رمزًا (اختياري)",
    },
    classBoard: {
      open: "الترتيب",
      title: "ترتيب الصف",
      desc: "كل الطلاب في جميع المسابقات المرتبطة بهذا الصف.",
      empty: "لم يشارك هذا الصف أي مسابقة بعد.",
    },
    bulk: {
      open: "لصق متعدد",
      title: "إضافة أسئلة دفعة واحدة",
      desc: "سؤال في كل سطر. افصل الخيارات بـ | وضع * بعد الإجابة الصحيحة.",
      label: "الأسئلة",
      hint: "حقلان يصنعان سؤالًا كتابيًا؛ ثلاثة أو أكثر تصنع اختيارًا من متعدد.",
      add: "إضافة الأسئلة",
      added: "تمت إضافة {count} سؤالًا",
      nothingParsed: "تعذّرت قراءة أي سؤال من النص.",
    },
    openGames: {
      inProgress: "بدأت بالفعل",
      title: "المسابقات المفتوحة الآن",
      desc: "اضغط على مسابقة لتعبئة رمزها، أو اكتب الرمز بالأسفل.",
      count: "{count} مفتوحة",
    },
    recite: {
      modeFollow: "تابع التلاوة",
      modeFill: "اتلُ واملأ الفراغات",
      modeDictation: "إملاء",
      modeRange: "تلاوة من … إلى …",
      repeats: "التكرار",
      repeatsLeft: "{count} مرة متبقية",
      playlist: "المتابعة للصفحة التالية",
      ayahs: "آية:",
      wholePage: "الصفحة كاملة",
      resume: "استئناف من {time}",
      title: "استمع وتابع",
      subtitle: "شغّل الصفحة وتابع كل كلمة أثناء تلاوتها. اضغط أي كلمة للبدء منها.",
      play: "تشغيل",
      pause: "إيقاف",
      restart: "من البداية",
      loop: "تكرار",
      hide: "إخفاء الكلمات",
      wordCount: "{count} كلمة",
      none: "لا توجد تلاوة منشورة لهذه الصفحة بعد.",
      failed: "تعذّر تحميل التلاوة.",
      tapHint: "اضغط على كلمة في الصفحة لتنتقل التلاوة إليها.",
    },
    range: {
      tapStart: "اضغط على الكلمة التي تبدأ منها.",
      tapEnd: "الآن اضغط على الكلمة التي تتوقف عندها.",
      selected: "{count} كلمة محدَّدة — اضغط أي كلمة لتحديد مقطع جديد.",
      play: "تلاوة المقطع",
      stop: "إيقاف",
      clear: "مسح",
      round: "الجولة {n} من {total}",
      speed: "السرعة",
      gap: "وقفة",
      wordByWord: "كلمة كلمة",
    },
    gen: {
      open: "توليد",
      title: "توليد أسئلة من صفحة",
      desc: "ينشئ تمارين من مواضع الكلمات وتوقيت التلاوة المنشورة.",
      run: "توليد",
      count: "العدد",
      wordsPer: "الكلمات المخفية",
      kindHidden: "كلمات مخفية",
      kindContinue: "ما الذي يأتي بعد",
      kindListen: "استمع وأجب",
      hasWords: "{count} كلمة محددة",
      noWords: "لا توجد بيانات كلمات لهذه الصفحة",
      hasAudio: "التلاوة متوفرة",
      noAudio: "لا توجد تلاوة",
      noData: "لا توجد بيانات كلمات لهذه الصفحة.",
      noImage: "تعذّر قياس صورة الصفحة.",
      nothing: "تعذّر توليد أي سؤال من هذه الصفحة.",
      added: "تم توليد {count} سؤالًا",
    },
    hifz: {
      mark: "تحديد التقدم",
      learning: "قيد الحفظ",
      known: "محفوظة",
    },
    answers: {
      open: "عرض الإجابات",
      title: "إجابات الطلاب",
      desc: "ما أجاب به كل لاعب. تمارين الصفحة تُعرض على الصفحة نفسها.",
      none: "لم يجب أحد على هذا السؤال بعد.",
      correct: "صحيحة",
      incorrect: "خاطئة",
      blank: "— بدون إجابة —",
      wrote: "كتب",
      expected: "المتوقع",
    },
    fill: {
      start: "ابدأ التمرين",
      restart: "فراغات جديدة",
      progress: "{done} من {total}",
      finished: "اكتملت الصفحة",
      whatWord: "توقفت التلاوة هنا — ما الكلمة التالية؟",
      check: "تحقق",
      reveal: "أظهر الكلمة",
      notThat: "ليست هذه الكلمة. استمع مرة أخرى وحاول.",
      hint: "يتوقف الصوت عند كل كلمة مغطاة وينتظرك.",
    },
    dictation: {
      listen: "استمع للآية",
      next: "الآية التالية",
      ayahOf: "الآية {n} من {total}",
      placeholder: "اكتب ما تسمعه…",
      check: "تحقق",
      score: "{score} من {total} كلمة صحيحة",
    },
    replay: {
      hearPage: "استمع إلى تلاوة هذه الصفحة",
    },
    quick: {
      joinDesc: "أدخل الرمز، أو اختر مسابقة مفتوحة الآن.",
      classesDesc: "صفوفك والمسابقات التي أقامها معلمك.",
      practiceDesc: "استمع وتابع الكلمات واختبر نفسك.",
    },
    classGames: {
      closed: "أغلق المعلم الدخول",
      open: "المسابقات",
      title: "مسابقات الصف",
      desc: "المسابقات التي أقامها معلمك لهذا الصف. لا حاجة إلى رمز.",
      none: "لم يقم هذا الصف بأي مسابقة بعد.",
      live: "مفتوحة الآن",
      enter: "دخول",
      rejoin: "العودة",
      results: "النتائج",
      enterFailed: "تعذّر الدخول إلى هذه المسابقة",
    },

    series: {
      title: "سلاسل التمارين",
      subtitle: "سلاسل أعدّها المعلّم. كل تمرين يُخفي بضع كلمات من صفحة واحدة.",
      none: "لا توجد سلاسل",
      noneDesc: "لم ينشر المعلّم أي سلسلة بعد.",
      exercises: "تمارين",
      exercise: "التمرين {num}",
      page: "الصفحة",
      difficulty: "الصعوبة",
      words: "الكلمات",
      minutes: "{count} د",
      noLimit: "بلا حدّ زمني",
      modeWrite: "اكتب الكلمة",
      modePosition: "حدّد الموضع",
      writeHint: "لكل موضع أدناه، اكتب الكلمة التي فيه.",
      positionHint: "لكل كلمة أدناه، حدّد رقم سطرها وترتيبها داخل السطر.",
      orderNote: "تُعدّ الأسطر من أعلى الصفحة، ويُعدّ الترتيب من يمين السطر.",
      start: "ابدأ",
      startAgain: "أعد المحاولة",
      finish: "سلّم الإجابة",
      timeLeft: "الوقت المتبقّي",
      elapsed: "الوقت",
      lineNo: "السطر",
      rankNo: "الترتيب",
      word: "الكلمة",
      yourAnswer: "إجابتك",
      expected: "الصواب",
      blank: "تُركت فارغة",
      score: "النتيجة",
      correctCount: "{right} صحيحة من {total}",
      done: "تمّ التصحيح",
      backToList: "العودة إلى السلاسل",
      startFailed: "تعذّر فتح هذا التمرين",
      submitFailed: "تعذّر تسليم الإجابة",
      best: "الأفضل",
      notYet: "لم يُنجز بعد",
      attemptsCount: "{count} محاولات",
      reportTitle: "نتائج السلاسل",
      reportSubtitle: "مستوى كل تلميذ في القسم، سلسلة بسلسلة.",
      chooseClass: "القسم",
      noClasses: "أنشئ قسمًا أولًا: النتائج مرتّبة حسب الأقسام.",
      student: "التلميذ",
      average: "المعدّل",
      lastAttempt: "الأخيرة",
      noAttempts: "لم يسلّم أحد في هذا القسم أي سلسلة بعد.",
      recent: "آخر المحاولات",
      time: "الوقت",
      mistakes: "الأخطاء",
    },
};

export default ar;
