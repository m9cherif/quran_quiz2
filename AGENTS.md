...
## Supabase project state (Phases 4–6 done, 2026-08-10)

- Schema: `profiles` (auth users, role `host|student`, auto-created by trigger from `app_metadata`) + `competitions` (now with `owner_id`, `visibility`, `cover_url`, `language`, `category`, `difficulty`, `archived_at`; join codes auto-generated by trigger), `participants`, `questions`, `choices`, `answers`, `admin_keys`. FK cascades: questions/choices/participants/answers follow the quiz; `answers.choice_id → set null`.
...
- Client-facing DB surface is RPC-only where it matters: `join_competition`, `submit_answer`, `get_question_reveal`, `game_leaderboard`, plus quiz management (`save_question`, `get_question_full`, `list_my_quizzes`, `archive_quiz`, `duplicate_quiz` — owner-guarded, security-definer WARNs in advisors are by design). Direct inserts into `participants`/`answers` are revoked. Consequence: the quiz editor reads/writes questions ONLY through these RPCs — never via direct table access.
...
- Next.js 15 App Router + React 18. Routing now real (Phase 2 done): `/` landing, `/about`, `/join` (+`/join/[code]`), `/login`, `/register`, `/host`, `/host/quizzes` (library) + `/new` + `/[id]/edit` (editor), `/host/games` (live games) + `/[roomKey]` live control, `/game/[code]` (lobby) + `/game/[code]/question`, plus `/design-system` (QA gallery).
